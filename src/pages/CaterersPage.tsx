...
import { Caterers } from "../data/caterersData";
import type { Caterer } from "../data/caterersData";
import { Link } from "react-router-dom";

const CaterersPage = () => {
  const [filteredData, setFilteredData] = useState<Caterer[]>(Caterers);

  const filterByLocation = (location: string) => {
    if (location === "All") {
      setFilteredData(Caterers);
      return;
    }

    const filtered = Caterers.filter((caterer) => caterer.city === location);
    ...
    <div className="grid grid-cols-1 md:grid-cols-[16rem_1fr] gap-6 mt-10">
      ...
      <main className="p-6">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
          {filteredData.map((caterer) => (
            <Link to={`/find-caterers/${caterer.id}`} key={caterer.id}>
              <div className="bg-white shadow-md rounded-2xl overflow-hidden hover:shadow-lg transition cursor-pointer">
                ...
                <div className="p-4">
                  ...
                  <p className="text-gray-500">📍 {caterer.city}</p>

                  <p className="text-yellow-500 mt-1">⭐ {caterer.rating}</p>
