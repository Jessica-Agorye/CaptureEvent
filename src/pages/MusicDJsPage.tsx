...
import type { MusicDJ } from "../data/musicDjData";
import { Link } from "react-router-dom";

const MusicDJsPage = () => {
  const [filteredData, setFilteredData] = useState<MusicDJ[]>(MusicDJs);

  const filterByLocation = (location: string) => {
    if (location === "All") {
      setFilteredData(MusicDJs);
      return;
    }

    const filtered = MusicDJs.filter((dj) => dj.city === location);
    ...
    <div className="grid grid-cols-1 md:grid-cols-[16rem_1fr] gap-6 mt-10">
      ...
      <main className="p-6">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
          {filteredData.map((dj) => (
            <Link to={`/find-music-djs/${dj.id}`} key={dj.id}>
              <div className="bg-white shadow-md rounded-2xl overflow-hidden hover:shadow-lg transition cursor-pointer">
                ...
                <div className="p-4">
                  ...
                  <p className="text-gray-500">📍 {dj.city}</p>

                  <p className="text-yellow-500 mt-1">⭐ {dj.rating}</p>

                  <p className="text-indigo-600 font-medium mt-2">{dj.price}</p>
                </div>
              </div>
            </Link>
          ))}
