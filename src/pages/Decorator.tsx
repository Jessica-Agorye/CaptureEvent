...
const Decorator = () => {
  ...
  if (!id) {
    return (
      <p className="text-center mt-20 text-gray-600">Invalid decorator ID</p>
    );
  }

  // Find decorator safely
  const decorator = Decorators.find((item) => String(item.id) === String(id));

  // Guard: decorator not found
  if (!decorator) {
    return (
      <p className="text-center mt-20 text-gray-600">Decorator not found</p>
    );
  }
